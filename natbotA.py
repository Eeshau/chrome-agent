...
Don't try to type into a link only text areas and input fields.
...
do not confuse <link> for an input

the locator should be based on the address provided

Don't try to interact with elements that you can't see.

DO NOT SIGN UP FOR ANYTHING, Avoid clicking on promotional popups, try to exit them or go back if encountered.
...
#		) * 100
		...
		for index, node_name_index in enumerate(node_names):
			...
			if node_name == "#text" and ancestor_exception:
				...
				if (
					(node_name == "input" or node_name == "textarea" and element_attributes.get("type") == "submit")
					or node_name == "button"
				):
					node_name = "button"
...
if (
	...
	anthropic = Anthropic()
	def get_gpt_command(objective, url, previous_command, browser_content, full_html_content):
		prompt = prompt_template
		prompt = prompt.replace("$objective", objective)
		...
		prompt += "\nFULL HTML CONTENT:\n" + full_html_content
		prompt += "\nThe task is to order a pizza from a specific pizza place. If the current page is a search engine, the next command should be to search for the pizza place. Please provide the next command in the format: [action] [id]."
		...
		# Post-process the model's output to extract only the command
	...
	print("\nWelcome to natbot! What is the pizza place name")
	i = input()
	objective = f"delivery order ONLY a medium cheese pizza from {i} to 151 Charles St W Suite 100, Kitchener, ON N2G 1H6. Address Type is a house. Street Address is 151 Charles St W Suite 100. City is Kitchener. Provience is ON. Postal code is N2G 1H6. If it asks for an email or first name last name use john doe and johndoe@gmail.com .Ignore or exit all other popups about subscribing or special deals, it might help you in ignoring these popups to consider the elements in relation to one another; an elements with adjacent ids might provide important context to what the element does in relation to the whole page. the output should exclusively be the [action] [id] . For forms if a text area of id n doesn't work try text area of id n+1 as you might be confusing a label for an input field"
	if len(i) > 0:
		pizza_place = i
	...
	try:
		while True:
			browser_content = "\n".join(_crawler.crawl())
			prev_cmd = gpt_cmd
			full_html_content = _crawler.page.content()  # Get the full HTML content
			soup = BeautifulSoup(full_html_content, 'html.parser')

			for tag in black_listed_elements:
				[s.extract() for s in soup(tag)]

			full_html_content = str(soup)


			gpt_cmd = get_gpt_command(objective, _crawler.page.url, prev_cmd, browser_content, full_html_content)
